...
const LedgerScreen: React.FC<LedgerScreenProps> = ({ transactions, onBack, onDelete, onImport }) => {
  ...
  const handleExport = () => {
    ...
    const fromTime = formatTimeForFile(sortedByDate[0].timestamp);
    const toTime = formatTimeForFile(sortedByDate[sortedByDate.length - 1].timestamp);
    const fileName = `fixpay_ledger_from_${fromTime}_to_${toTime}.csv`;
    ...
    reader.onload = (evt) => {
      try {
        ...
        for (let i = startIndex; i < lines.length; i++) {
          ...
          // Handle potential quotes in CSV if generated by other tools, 
          // but for this simple implementation we assume simple split.
      ...
      <div className="max-w-md mx-auto w-full relative z-10 flex flex-col h-full">
        ...
        <div className="flex justify-between items-center mb-8 mt-2">
          ...
          <div className="flex gap-2">
             ...
             <button onClick={handleImportClick} className="flex items-center gap-2 px-3 py-2 border-4 border-black bg-white hover:bg-gray-100 transition-all text-black shadow-neo-sm active:shadow-none active:translate-x-1 active:translate-y-1" title="Import CSV">
                <FileUp size={20} strokeWidth={2.5}/>
                <span className="font-black text-xs uppercase hidden sm:inline">Import</span>
             </button>
             <button onClick={handleExport} className="flex items-center gap-2 px-3 py-2 border-4 border-black bg-white hover:bg-gray-100 transition-all text-black shadow-neo-sm active:shadow-none active:translate-x-1 active:translate-y-1" title="Export CSV">
                <FileDown size={20} strokeWidth={2.5}/>
                <span className="font-black text-xs uppercase hidden sm:inline">Export</span>
             </button>
          </div>
        </div>
